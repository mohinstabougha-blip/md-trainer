        ...
        {MARKTPLATZ_BEISPIELE.map((b, i) => (
          <div key={i} className="kp-card flex flex-col gap-1 opacity-90">
            <div className="flex items-start justify-between gap-2">
              <span className="font-medium text-zinc-700 dark:text-zinc-300">{b.titel}</span>
              <span className="shrink-0 rounded-full bg-zinc-200 px-2 py-0.5 text-[11px] font-medium text-zinc-600 dark:bg-zinc-700 dark:text-zinc-300">
                Beispiel
              </span>
            </div>
            <p className="line-clamp-3 text-sm text-zinc-600 dark:text-zinc-400">{b.beschreibung}</p>
