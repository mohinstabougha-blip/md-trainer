...
export function SucheBar() {
  ...
  return (
    <div className="mx-auto w-full max-w-xl px-6 pt-4">
      ...
      {status !== "leer" && (
        <div className="mt-2 max-h-[70vh] overflow-y-auto rounded-2xl border border-zinc-100 bg-white p-2 shadow-sm">
          {status === "laden" && (
            <p className="px-2 py-3 text-sm text-zinc-500">Suche läuft…</p>
          )}
          {status === "fehler" && (
            <p className="px-2 py-3 text-sm text-red-700">Das hat nicht geklappt.</p>
          )}
          ...
          {status === "fertig" &&
            treffer.map((t) => {
              ...
              return (
                <div key={t.id} className="border-b border-zinc-100 p-2 last:border-b-0">
                  ...
                  {istOffen && (
                    <div className="mt-2 flex flex-col gap-2 rounded-xl bg-violet-50/50 p-2">
